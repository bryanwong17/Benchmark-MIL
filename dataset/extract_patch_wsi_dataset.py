...
class ExtractFeaturesWSIDataset(Dataset):
    def __init__(self, dataset_root, dataset_name, dataset_df, data_mean, data_std,
                 class_names_list, magnification, size, batch_patch_size=100):
        super().__init__()
    ...
    def __getitem__(self, idx):
        ...
        # class_name, slide_id, caption = row["class_name"], row["slide_id"], row["caption"]
        class_name, tissue_id = row["class_name"], row["slide_id/tissue_name"]

        # slide_dir = f"{self.dataset_root}/{self.dataset_name}/merge_patch/{class_name}/{slide_id}"
        tissue_dir = f"{self.dataset_root}/MergeTissue/{self.magnification}/{self.size}/{class_name}/{tissue_id}"
        merge_patch_paths = list(Path(tissue_dir).glob('*'))

        # original_slide_dir = f"{self.dataset_root}/patches/{self.magnification}/{self.size}/{class_name}/{slide_id}" #
        # original_patch_paths = list(Path(original_slide_dir).glob('*.png')) #
        ...
        return class_name, tissue_id, patches